...
The CK Editor system has been monetized and completely changed.
...
* Includes a self-hosted Tiny MCE (GPL edition) for full-featured editing.
...
* Replaced CK Editor with a self-hosted Tiny MCE, loaded from `/tinymce/tinymce.min.js` (installed via the `tinymce/tinymce` composer package)
* Generates a one-time token (stored in `$_SESSION['token']`) and passes its hash to Tiny MCE's `images_upload_url` so `/super/upload` can verify the request came from this form
#### `/src/config/config.php`
* Renamed the `SUPER` => `ckeditor` key to `tinymce`
#### `/src/upload.php`
* Checks the token generated by `edit.phtml`; if it doesn't check out, redirects to the main page
* Reshapes the response from `FileCMS\Common\File\Upload::handle()` into the `{location: url}` format Tiny MCE's image upload handler expects, and returns HTTP 400 with `{error: message}` on failure
#### `composer.json`
* Added `tinymce/tinymce` as a dependency
